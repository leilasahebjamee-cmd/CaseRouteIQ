import { createCase, getCustomer, getCasesForOrganization, getSelfHelpArticles } from './customer-store.js';
...
function findRelatedCase(customer, request) {
  const candidates = getCasesForOrganization(customer.organizationId)
    ...
    .map((supportCase) => ({ supportCase, score: similarity(request.summary, supportCase.summary) + (request.category === supportCase.category ? 0.25 : 0) }))
    .sort((a, b) => b.score - a.score);
  const match = candidates[0];
  return match && match.score >= 0.5 ? match : null;
}
function selfHelpFor(request) { return getSelfHelpArticles(request.category).map(({ id, title, url, summary }) => ({ id, title, url, summary })); }
...
export function assessSupportRequest(request) {
  ...
  return {
    ...
    entitlement: { serviceLevel: customer.supportTier, requestedChannel: request.channel, channelEligible, eligibleChannels: customer.eligibleChannels, responseTargetHours: customer.responseTargetHours },
    selfHelp: { recommended: selfHelp.length > 0, articles: selfHelp, message: selfHelp.length > 0 ? 'Try these self-help articles before opening a new case. You can still contact support if they do not resolve the issue.' : 'No matching self-help article is available; continue with support triage.' },
    issue: related
    ...
    nextStep: channelEligible ? (related ? 'Confirm the details and add this report to the existing case.' : selfHelp.length > 0 ? 'Present the self-help articles. If unresolved, create a new support case using the eligible service level.' : 'Create a new support case using the eligible service level.') : `Ask the customer to use one of: ${customer.eligibleChannels.join(', ')}.`
  };
}

export function createSupportCase(request) {
  const assessment = assessSupportRequest(request);
  ...
  const customer = getCustomer(request.customerId);
  const supportCase = createCase(customer, request);
  return { status: 'created', supportCase, assessment };
}
