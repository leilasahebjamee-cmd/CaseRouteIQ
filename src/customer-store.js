...
export function getCustomer(customerId) { return customers.get(customerId) ?? null; }
export function getCasesForOrganization(organizationId) { return cases.filter((supportCase) => supportCase.organizationId === organizationId); }
export function getSelfHelpArticles(category) { return selfHelpArticles.filter((article) => article.category === category); }

export function createCase(customer, request) {
  const supportCase = {
    id: `case-${String(cases.length + 480).padStart(3, '0')}`,
